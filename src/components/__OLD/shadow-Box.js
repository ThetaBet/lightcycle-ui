import { html } from '../../utils/tag';
import BaseComponent from '../base-component';

export default class ShadowBox extends BaseComponent {
  constructor() {
    super();
  }

  setCss() {
    return html`
      :host {
        --spread-min: 0px;
        --spread-max: 24px;
        --h: 1;
        --shadow-length: calc((1 - var(--lc-light-intensity)) * 20px + 4px);
        --shadow-x: calc(var(--lc-light-x) * var(--shadow-length));
        --shadow-y: calc(var(--lc-light-y) * var(--shadow-length));
        --inv-elev: calc(1 - var(--sun-elevation));
        --inv-elev-2: calc(var(--inv-elev) * var(--inv-elev));
        --inv-elev-3: calc(var(--inv-elev-2) * var(--inv-elev));
        --spread: calc(var(--inv-elev-2) * var(--spread-max));
        --blur: calc(var(--spread) * 1.2); 
        --light-warmth: calc(1 - var(--sun-elevation));
        --hue: calc(260 - var(--sun-elevation) * 40);
        --lightness: calc(30% - var(--sun-elevation) * 10%);
        --saturation: calc(30% - var(--sun-elevation) * 15%);
        --reflect-x: calc(var(--lc-light-x) * -10px);
        --reflect-y: calc(var(--lc-light-y) * -10px);
        // box-shadow: var(--shadow-x) var(--shadow-y) var(--blur) var(--spread) rgba(60, 80, 120, calc(0.4 - var(--sun-elevation) * 0.2));
        filter: drop-shadow(var(--shadow-x) var(--shadow-y) var(--blur) rgba(60, 80, 120, calc(0.4 - var(--sun-elevation) * 0.2)));
        position: relative;
        overflow: hidden;
        width: 100%;
        height: 100%;
      }
    `;
  }
}
